...
function App() {
  ...
  const handlePreviewDrag = (e) => {
    if (dragging.current === 'preview' && mainContentRef.current) {
      ...
      let newWidth = Math.max(PREVIEW_MIN_WIDTH, Math.min(total - offsetX, PREVIEW_MAX_WIDTH));
  ...
  return (
    <div className="flex h-screen bg-white select-none w-full" style={{ minWidth: 0 }}>
      ...
      <div ref={mainContentRef} className="flex flex-row flex-1 min-w-0 h-full w-full">
        ...
        <div
          ...
          className={`w-2 cursor-col-resize bg-transparent hover:bg-blue-200 transition${previewAtMin ? ' pointer-events-none opacity-30' : ''}`}
          style={{ zIndex: 20 }}
        >
          <div className="mx-auto h-full w-1 bg-gray-200 rounded" />
        </div>
        {/* Preview panel with enforced min/max width, no shrink */}
        <div style={{ width: previewWidth, minWidth: PREVIEW_MIN_WIDTH, maxWidth: PREVIEW_MAX_WIDTH, flexShrink: 0, transition: 'width 0.1s' }}>
